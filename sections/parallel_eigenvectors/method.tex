...
\begin{algorithm}[p]
    ...
    \Fn{\FindPEV{$\triangle_{\mS}$, $\triangle_{\mT}$, $\triangle_{\vx}$}}{
        $\vr \gets$ \FindEigenDir{$\triangle_{\mS}$, $\triangle_{\mT}$}\;
        \uIf{$\vr$ is \KwNull}{
            \Return{$\{\}$}
            \tcp*{Discard triangle}
        }
        \ElseIf{size of $\triangle_{\vx} < \varepsilon_{\mathrm{s}}$}{
            \Return{$\{(\sfrac{1}{3}\sum{\vx_i}, \ \vr)\}$}
            \tcp*{Accept solution}
        }
        $l$ = \{\}\;
        \ForEach{$(\triangle'_{\mS}, \triangle'_{\mT}, \triangle'_{\vx}) \in (\text{%
               \Split{$\triangle_{\mS}$},
                \Split{$\triangle_{\mT}$},
                \Split{$\triangle_{\vx}$}})$}{
            $l \gets l \ \cup$
                \FindPEV{$\triangle'_{\mS}$,
                         $\triangle'_{\mT}$,
                         $\triangle'_{\vx}$}
    ...
    % \BlankLine
    \Fn{\FindEigenDir{$\triangle_{\mS}$, $\triangle_{\mT}$}}{
        $R \gets$ set of triangles covering a hemisphere\;
        \ForEach{$\triangle_{\vr} \in R$}{
            $\vr \gets$ \FindEigenDirRecursive{$\triangle_{\mS}$,
                                               $\triangle_{\mT}$,
                                               $\triangle_{\vr}$}\;
            \If{$\vr$ is not \KwNull}{
                \Return{$\vr$}\;
    ...
    % \BlankLine
    \Fn{\FindEigenDirRecursive{$\triangle_{\mS}$,
                               $\triangle_{\mT}$,
                               $\triangle_{\vr}$}}{
        Compute Bernstein-B\'ezier coefficients for $\hat{w}_i$\;
        \uIf{any $\hat{w}_i$ might have roots within $\triangle_{\vr}$}{
            \ForEach{$\triangle'_{\vr} \in$ \Split{$\triangle_{\vr}$}}{
               $\vr \gets$ %
                    \FindEigenDirRecursive{$\triangle_{\mS}$,
                                           $\triangle_{\mT}$,
                                           $\triangle'_{\vr}$}\;
                \If{$\vr$ is not \KwNull}{
                    \Return{$\vr$}\;
        ...
        \uElseIf{all $\hat{w}_i$ have the same sign %
                \KwOr size of $\triangle_{\vr} < \varepsilon_{\mathrm{r}}$}{
