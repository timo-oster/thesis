...
across the flame front. \Todo{Mention pressure/density and velocity and that
they need to be treated separately.} This direction is approximately orthogonal
...
\begin{figure}[t]
	...
	\Todo[inline]{Fix description, possibly change color map, annotate normals
	$\vn$ and label $t$ on plots}
\end{figure}
%
...
\begin{figure}[tb]
	\centering
	\MissingFigure{Points seeded on real surface}
...
\vr_i$, with $||\vr_i|| = 1$ \Todo{use $\vn$ as normal} (\cref{fig:flamecrossing}).
%
...
\Todo{Slope of profile is useful for determining mesh resolution for simulation (\cite{Poinsot2012}, p. 57)}
%
\begin{equation}
...
\end{equation}
%
where $(x_m, y_m)$\Todo{Make use of $t$ for parameterization along profile line
and $x, y$ for parameters of models consistent} is the location of the
...
\cref{fig:flamecrossing,fig:models}), we can find more feature
