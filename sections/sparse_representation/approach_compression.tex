...
and \ref{i:visualize}, we introduce a sparse representation for DNS data. The
...
across the flame front. This direction is approximately orthogonal to the flame
surface. We exploit this by representing the 3D scalar fields of the
variables as a set of profiles on 1D lines. These profile lines are anchored at
points $\vp_i$ on the flame surface and oriented along its normal
...
seeding more lines in highly curved areas and less in planar regions (see Figure
\ref{fig:flamecrossing}).
...
\begin{figure}[t!]
	\begin{center}
		\includegraphics[width=\linewidth]{figures/flamecrossing.pdf}
	\end{center}
	\caption{
	...
	\ruggedtodo[inline]{Recreate in tikz, don't use colored boxes}
\end{figure}
%
...
CGAL~\cite{Boissonnat2005}. This yields a mesh with approximately uniformly-spaced
vertices and edges of approximately the same length as the voxel size in
the original data.
%
%We first place a point into every surface voxel. 
%
% Sub-voxel sampling would not yield more information due to the smoothness of DNS
% data. 
%
...
% $\seedDens$ ensures that medium curvatures, which are most frequent, are not
...
% size.
% 
Due to the high resolution and accuracy of DNS data, trilinear
...
%----
% Even though DNS data are simulated without any model assumptions,
...
% the simulation benefits of DNS data being that DNS data have a low
...
%----
Although the main advantage of DNS is its high precision due to the lack of any
...
\begin{figure}[t!]
	\centering
	\setlength\figurewidth{0.88\linewidth}
...
% 	\end{center}
% 	\caption{
% 	Top: Fitting the sigmoidal model to a profile of temperature.
...
% The parameter matrix $\Phi^V_{\text{rp}}$ of the sigmoidal model for reactants
% and products is
% \begin{equation*}
% \Phi^V_{\text{rp}} = 
...
% The parameter matrix $\Phi^V_{\text{int}}$ of the Gaussian model for 
