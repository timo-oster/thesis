...
\begin{figure}[p]
	...
	% Top: Graphical user interface.
	% Upper middle: %($S^{\text{HEATR}}_{\textnormal{max}}$ vs. $S^{\text{TEMP}}_{\textnormal{infl}}$)
	% %(Maximum surface of heat release vs. inflection point surface of temperature)
	% Effect of varying color scale parameter $u_1$.
	% Lower middle: %($S^{\text{HEATR}}_{\textnormal{max}}$ vs. $S^{\text{H}_2\text{O}_2}_{\textnormal{max}}$)
	...
	% Bottom: %($S^{\text{TEMP}}_{\textnormal{infl}}$ vs. $S^{\text{O}}_{\textnormal{max}}$)
