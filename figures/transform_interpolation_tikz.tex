%
%
\begin{tikzpicture}[scale=\figurewidth/1cm*0.13,
					line join=round]
\tikzstyle{ghostparticle} = [thick, fill=white, arrows=-o]
\tikzstyle{b1style} = [thick, red, arrows=-latex]
\tikzstyle{b2style} = [thick, blue, arrows=-latex]
\tikzstyle{b1dstyle} = [very thick, red!50, arrows=-latex]
\tikzstyle{b2dstyle} = [very thick, blue!50, arrows=-latex]
\tikzstyle{ellipse1style} = [thick, dashed, cyan!70]
\tikzstyle{ellipse2style} = [thick, dashed, orange!50]
...
% normal
% \draw [thick, fill=black, arrows=-latex]
...
\draw [thick, fill=black] (vx) circle [radius=0.08]
...
\begin{scope}[shift={(3.5, 0)}]
	...
	\draw [thick, fill=black] (vx2) circle [radius=0.08]
		node [right, xshift=0.05cm] {$\vx'$};
\end{scope}

\draw [very thick, cyan!70, shorten <=0.6cm, shorten >=0.6cm, arrows=-latex]
		(center1) to[out=-45, in=225]
			node [below, midway, text=black, yshift=-0.1cm] {$\mE$}
		(center2);

\draw [very thick, orange!50, shorten <=0.6cm, shorten >=0.7cm, arrows=-latex]
		(vx) to[out=25, in=145]
