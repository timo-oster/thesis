...
\begin{tikzpicture}[
        ...
        image/.style={anchor=center, inner sep=0, outer sep=0,
                      node distance = 1.3cm and 0,
                      minimum width=\figurewidth},
        label/.style={anchor=north, align=left, yshift=-1mm},
        dashline/.style={dashed, thick, shorten <=-2mm, shorten >=-2mm}
    ]
    \input{colormap_cooltowarm_extended}
    \input{colormap_green_purple}
    \node[image] (image1) {
        \begin{tikzpicture}
            \node[image] (base) at (0,0) {
                \includegraphics[width=0.8\figurewidth] {figures/hawkes_time_3e-5}
            };%
            \begin{scope}
                \clip (base.south east) --
                    ($(base.south west)!0.4!(base.south east)$) --
                    ($(base.north west)!0.6!(base.north east)$) --
                    (base.north east) --
                    cycle;
                \node[image] at (0,0) {
    ...
    \node[label] at (image1.south) {
        $t = \SI{3e-5}{\second} \quad N=\num{160}\si{\kilo}$
    };

    \node[image, below=of image1] (image2) {
        \begin{tikzpicture}
            \node[image] (base1) at (0,0) {
                \includegraphics[width=0.8\figurewidth] {figures/hawkes_time_4e-5}
            };%
            \begin{scope}
                \clip (base1.south east) --
                    ($(base1.south west)!0.4!(base1.south east)$) --
                    ($(base1.north west)!0.6!(base1.north east)$) --
                    (base1.north east) --
                    cycle;
                \node[image] at (0,0) {
    ...
    \node[label] at (image2.south) {
        $t = \SI{4e-5}{\second} \quad N=\num{670}\si{\kilo}$
    };

    \node[image, below=of image2] (image3) {
        \begin{tikzpicture}
            \node[image] (base2) at (0,0) {
                \includegraphics[width=0.8\figurewidth]{figures/hawkes_time_5e-5}
            };%
            \begin{scope}
                \clip (base2.south east) --
                    ($(base2.south west)!0.4!(base2.south east)$) --
                    ($(base2.north west)!0.6!(base2.north east)$) --
                    (base2.north east) --
                    cycle;
                \node[image] at (0,0) {
    ...
    \node[label] at (image3.south) {
        $t = \SI{5e-5}{\second} \quad N=\num{3000}\si{\kilo}$
    };

    \coordinate (bottom) at ([yshift=-2.5cm]image3.south);

    \clip ([yshift=3mm]image1.north west) rectangle (image3.east |- bottom);

    \node[anchor=north east, yshift=-1.1cm, xshift=-6mm] at (image3.south){
        \begin{axis}[
        scale only axis,
        height=4cm,
        hide axis,
    ...
    \node[anchor=north west, yshift=-1.1cm, xshift=-6mm] at (image3.south){
        \begin{axis}[
        scale only axis,
        height=4cm,
        hide axis,
